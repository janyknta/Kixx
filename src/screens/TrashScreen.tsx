...
const TrashScreen: React.FC<TrashScreenProps> = ({ onBack, onItemRestored }) => {
  ...
  const loadTrashItems = useCallback(async () => {
    try {
      ...
      if (!loadResult.success) {
        showError(loadResult.error || 'Failed to load trash data');
        return;
      }
    ...
    try {
      if (targetItemId) {
        ...
        if (result.success) {
          ...
          showError(result.error || 'Failed to restore item');
        }
      } else {
        // Batch restore
        ...
        const itemText = result.processed === 1 ? 'item' : 'items';
        if (result.errors.length > 0) {
          showWarning(`Restored ${result.processed} ${itemText} with ${result.errors.length} error${result.errors.length !== 1 ? 's' : ''}`);
        } else {
          showSuccess(`Restored ${result.processed} ${itemText} successfully`);
        }
      ...
      showError('Failed to restore item(s)');
    } finally {
  ...
  const confirmDelete = async () => {
    ...
    try {
      if (targetItemId) {
        ...
        const itemText = result.processed === 1 ? 'item' : 'items';
        if (result.errors.length > 0) {
          showWarning(`Permanently deleted ${result.processed} ${itemText} with ${result.errors.length} error${result.errors.length !== 1 ? 's' : ''}`);
        } else {
          showSuccess(`Permanently deleted ${result.processed} ${itemText}`);
        }
      ...
      showError('Failed to delete item(s)');
    } finally {
      setTargetItemId(null);
    }
  };
  ...
  const handleEmptyTrash = useCallback(() => {
    ...
    showAlert({
      title: 'Empty Trash',
      message: `Permanently delete all ${trashItems.length} items in trash?`,
      icon: 'delete-forever',
  ...
  }, [trashItems, mediaService, showAlert, showSuccess, showError, loadTrashItems, colors.danger]);
  ...
  const getDialogProps = () => {
    ...
    if (confirmAction === 'restore') {
      ...
      return {
        ...
        message: `Permanently delete ${count} item${count !== 1 ? 's' : ''}? `,
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.vaultBackground }]}>
      {/* Header content moved to parent - add any controls here if needed */}
      {trashItems.length > 0 && (
        <View style={[styles.trashControls, { backgroundColor: colors.vaultSurface, borderBottomColor: colors.border }]}>
          <Text style={[styles.trashInfo, { color: colors.textSecondary }]}>
            {trashItems.length} items in trash • Items are permanently deleted after 30 days
          </Text>
          <View style={styles.controlsRow}>
            {!isSelectionMode ? (
              <>
                <TouchableOpacity 
                  style={[styles.selectButton, { borderColor: colors.vaultAccent }]} 
                  onPress={() => {
                    setIsSelectionMode(true);
                    setSelectedItems(new Set());
                  }}
                >
                  <Text style={[styles.selectButtonText, { color: colors.vaultAccent }]}>Select</Text>
                </TouchableOpacity>
                <TouchableOpacity 
                  style={[styles.emptyTrashButton, { backgroundColor: colors.danger }]} 
                  onPress={handleEmptyTrash}
                >
                  <Icon name="delete-forever" size={16} color={colors.surface} />
                  <Text style={[styles.emptyTrashText, { color: colors.surface }]}>Empty Trash</Text>
                </TouchableOpacity>
              </>
            ) : (
              <View style={styles.selectionControls}>
                ...
                <View style={styles.selectionButtons}>
                  ...
                  <TouchableOpacity onPress={handleBatchRestore} style={[styles.batchButton, { backgroundColor: colors.success }]}>
                    <Icon name="restore" size={16} color={colors.surface} />
                    <Text style={[styles.batchButtonText, { color: colors.surface }]}>Restore</Text>
                  </TouchableOpacity>
                  <TouchableOpacity onPress={handleBatchDelete} style={[styles.batchButton, { backgroundColor: colors.danger }]}>
                    <Icon name="delete-forever" size={16} color={colors.surface} />
                    <Text style={[styles.batchButtonText, { color: colors.surface }]}>Delete</Text>
                  </TouchableOpacity>
                </View>
...
const styles = StyleSheet.create({
  ...
  // New styles for bottom navigation integration
  trashControls: {
    backgroundColor: COLORS.vaultSurface,
    paddingHorizontal: 16,
    paddingVertical: 12,
